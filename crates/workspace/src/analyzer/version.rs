...
impl SyntaxVersions<'_, '_> {
    ...
    /// `PARALLEL` on `SELECT`/`CREATE`/`UPDATE`/`UPSERT`/`DELETE`/`RELATE`.
    ///
    /// The clause existed from 1.0 (`sql/statements/select.rs` and its
    /// siblings carry a `parallel: bool` at every 1.x and 2.x tag) and was
    /// deleted in 3.0 by surrealdb#6768, "Remove unused `PARALLEL` clause."
    /// — landed between `v3.0.0-beta.2` (the seven
