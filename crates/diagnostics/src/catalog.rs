...
const ENTRIES: &[(u16, &str, Severity, LintLevel)] = &[
    ...
    (1033, "a DEFINE FIELD/DEFINE INDEX clause is one its statement accepts (`id` rejects VALUE/READONLY/COMPUTED/DEFAULT ALWAYS; REFERENCE needs a record type; COMPUTED excludes DEFAULT/VALUE/READONLY/ASSERT and is never indexable; a COUNT index takes no FIELDS)", Severity::Error, Deny),
    ...
    (4034, "an aggregate is handed a column whose kind it cannot meaningfully aggregate", Severity::Warning, Warn),
