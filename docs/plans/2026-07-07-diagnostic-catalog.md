...
| 4030 | INSERT's RELATION and IGNORE modifiers are in the order the engine parses | `INSERT IGNORE RELATION INTO likes {…}` — the engine's parser eats `RELATION` then `IGNORE` (`syn/parser/stmt/insert.rs`, unchanged since RELATION arrived in 2.0), so the reversed spelling is ``Unexpected token `INTO`, expected Eof`` and no version of SurrealDB ever took it. The grammar accepts both orders on purpose: a parse error is fatal to the whole source, and "unexpected token" does not say what is wrong. Fires only when both keywords are present and `IGNORE` comes first | E | ✅ verified on a live 3.2.3 — `INSERT RELATION IGNORE INTO likes {…}` inserts, the reverse is a parse error |
