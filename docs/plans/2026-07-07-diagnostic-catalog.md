...
| 1022 | a definition does not redefine (OVERWRITE/IF NOT EXISTS state the intent) | two `DEFINE TABLE person` — SurrealDB 3.2 *rejects* the second outright, one message per kind: `The table 'person' already exists`, and likewise `field` / `index` / `event` / `function` / `param` / `analyzer` (all seven verified on 3.2.3). Nothing is replaced and nothing is silent, so this is E, not W; `OVERWRITE` (replace) and `IF NOT EXISTS` (keep the first) are the only two spellings the engine accepts. Also covers a cross-file ordering trap the per-statement checks cannot see on their own: a `DEFINE FIELD … ON t` registered before `t`'s own `DEFINE TABLE` (anywhere in the workspace) implicitly creates `t` schemaless, so that `DEFINE TABLE` — plainly, without `OVERWRITE`/`IF NOT EXISTS` — fails the same way, reported at the `DEFINE TABLE` | E | ✅ |
...
| 2004 | the operands make sense together for the operator | binary and unary, arithmetic and comparison, compound assignment (`age += 'x'`); SurrealDB kind-ordering instead of throwing changes nothing. Also inside an undeclared closure parameter a collection function applies to (`array::map(tags, \|$x\| $x + 1)` over `array<string>`) — the receiver's element kind is threaded into the parameter the same way it always was for typing the closure's *return*, now reaching the checking half too | E | ✅ emitting |
...
| 4033 | FOR iterating an inline SELECT subquery may fail depending on how many rows it matches | `FOR $x IN (SELECT * FROM user) {...}` — a bracketed subquery that matches exactly one row collapses to that row itself rather than a one-element array (the same convention `LET $x = (SELECT VALUE n FROM ONLY t LIMIT 1)` relies on to bind a scalar), and FOR cannot iterate it: "Cannot execute statement using value: user:1" on 3.2.3, with two-or-more matches (or zero) iterating normally. Genuinely data-dependent, so this is a "may", never a "does"; scoped to an inline subquery written directly in the iterable position — `LET $ids = (SELECT ...); FOR $u IN $ids` is a fixed value by the time FOR sees it and is not this check's concern | W | ✅ |
| 4034 | an aggregate is handed a column whose kind it cannot meaningfully aggregate | `SELECT math::sum(name) FROM t GROUP ALL` over a `string` column — none of this errors on 3.2.3: `math::sum` answers `0`, `math::mean` answers `NaN`, `math::max` answers `-Infinity`, `time::min`/`time::max` answer `NONE`. Scoped to a plain column reference (4028's own restriction), silent on `Any`/unresolved kinds and on a union carrying a member of the required family; `math::mode` is excluded — verified broken over a *numeric* column too (`[NONE, NONE, NONE]`), so a kind check has nothing useful to say about it | W | ✅ |
